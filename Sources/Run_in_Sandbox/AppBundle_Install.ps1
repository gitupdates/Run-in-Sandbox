...
foreach ($App in $Apps_to_install) {
    ...
    if ( ($App_File -like "*.exe*") -or ($App_File -like "*.msi*") ) {
        ...
        C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -sta -WindowStyle Hidden -NoProfile -ExecutionPolicy Unrestricted -File $Sandbox_Root_Path\IntuneWin_Install.ps1 -Intunewin_Content_File $Intunewin_Content_File -Intunewin_Command_File $Intunewin_Command_File -OpenDebugConsole True
    } else {
